...
//int weight[SIZE][SIZE] = { {7,5,3,2},{15,12,10,5},{23,20,12,7},{30,25,17,11} }; //stable
int init_depth;
...
int create_maxnode(PTree* T, int current_node, int* dir, int depth, int alpha, int beta)
{
	int i = 0, temp = 0, gameover_flag = 1;
	if (depth == 0)
	{
		alpha = get_value();
	}
	else
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				T->node_num = T->node_num + 1;
				temp = T->node_num;
				T->nodes[temp].parent = current_node;
				T->nodes[temp].value = create_minnode(T, temp, depth - 1, alpha, beta);
				

				if (T->nodes[temp].value >= alpha)
				{
					if (depth == init_depth)
						*dir = i;
					alpha = T->nodes[temp].value;
				}
				gameover_flag = 0;
			}
			array_cpy(depth, 1);
		...
		if (gameover_flag == 1)
		{
			...
			T->nodes[T->node_num].value = NGIF + 1;
		...
		for (i = 0; i < 4; i++)
		{
			for (j = 0; j < 4; j++)
			{
				if (game_board[i][j] == 0)
				{
					...
					temp = T->node_num;
					T->nodes[temp].parent = current_node;
					T->nodes[temp].value = create_maxnode(T, temp, &(T->dir), depth - 1, alpha, beta);

					if (T->nodes[temp].value < beta)
						beta = T->nodes[temp].value;
...
int DFS(int board[SIZE][SIZE])
{
	...
	SearchTree.node_num = 0;
	SearchTree.nodes[0].parent = -1;
	SearchTree.nodes[0].value = create_maxnode(&SearchTree, 0, &SearchTree.dir, depth, NGIF, PGIF);		//get the max value and its step
